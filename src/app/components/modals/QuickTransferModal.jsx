...
export function QuickTransferModal({ isOpen, onClose, onTransfer }) {
  ...
  const availableTo = isDebtPayment
    ? liabilityAccounts.filter(a => Math.abs(a.balance ?? 0) > 0)
    : accounts.filter(a => String(a.accountId) !== fromId);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-card rounded-2xl w-full max-w-md mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-border sticky top-0 bg-card rounded-t-2xl z-10">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-card-foreground">Chuyển khoản</h2>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-6 py-5 space-y-5">
            ...
            <div>
              ...
              >
                <option value="">
                  {isDebtPayment
                    ? "Chọn khoản nợ cần thanh toán"
                    : fromId ? "Chọn tài khoản đích" : "Chọn tài khoản nguồn trước"}
                </option>
                {availableTo.map(a => (
                  <option key={a.accountId} value={a.accountId}>
              ...
              {!isDebtPayment && fromId && availableTo.length === 0 && (
                <p className="text-xs text-amber-600 mt-1">Không còn tài khoản nào để chuyển.</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground font-medium text-sm">VND</span>
                <input
                  type="number"
                  value={amount}
          ...
          <div className="flex gap-3 px-6 py-4 border-t border-border">
            ...
            <button type="submit" disabled={!fromId || !toId || !amount || parseFloat(amount) <= 0}
