...
export function AddWalletModal({ isOpen, onClose, onAdd }) {
  ...
  return (
    ...
    >
      <div
        className="bg-card rounded-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto shadow-2xl"
          ...
          <div className="px-6 py-5 space-y-5">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`font-semibold text-sm ${isActive ? "text-purple-700" : "text-foreground"}`}
                        >
                          {type.label}
                        </p>
            ...
            <div>
              ...
              <input
                ...
                placeholder={
                  selectedKey === "default"
                    ? "VD: Tài khoản MB Bank"
                    : selectedKey === "shared"
                      ? "VD: Quỹ gia đình"
                      : selectedKey === "savings"
                        ? "VD: Tiết kiệm mua nhà"
                        : selectedKey === "credit"
                          ? "VD: Thẻ Visa Techcombank"
                          : selectedKey === "cash"
                            ? "VD: Ví tiền mặt"
                            : "Đặt tên tài khoản..."
                }
                  ...
                  {loadingSources ? (
                    ...
                    sourceAccounts.map((acc) => (
                      <option key={acc.accountId} value={acc.accountId}>
                        {acc.name} —{" "}
                        {Intl.NumberFormat("vi-VN").format(acc.balance ?? 0)}đ
                      </option>
                    ))
                  )}
                </select>
                {sourceAccountId && (
                  <div className="flex items-center gap-2 mt-2 p-2 bg-blue-50 rounded-lg text-xs text-blue-700">
                    ...
                    <span>
                      ...
                      đ sẽ được chuyển từ tài khoản đã chọn
                    </span>
                  </div>
                )}
              </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  setBalance(parseVND(e.target.value));
                  setSourceAccountId("");
                }}
                placeholder="0"
                min={0}
                ...
                {CURRENCIES.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span
                        className={`text-xs font-bold ${isActive ? "text-purple-700" : "text-muted-foreground"}`}
                      >
                        {c.code}
                      </span>
                      <span
                        className={`text-[10px] ${isActive ? "text-purple-500" : "text-muted-foreground"}`}
                      >
                        {c.symbol}
                      </span>
                    </button>
