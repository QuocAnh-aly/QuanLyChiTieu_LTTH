...
export function Budget() {
  ...
  const [filterStatus, setFilterStatus] = useState("all"); // all | on-track | warning | over
  const [sortBy, setSortBy] = useState("default"); // default | pct-desc | pct-asc | amount | name
  ...
  const handleDeleteBudget = async (id, name) => {
    ...
    try {
      ...
      await fetchBudgets();
      toast.success(`Đã xóa ngân sách "${name}".`);
    } catch {
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className={`rounded-2xl p-6 text-white ${remaining < 0 ? "bg-gradient-to-br from-red-500 to-red-700" : "bg-gradient-to-br from-green-500 to-green-700"}`}>
          ...
          <p className="text-3xl font-bold">${Math.abs(remaining).toLocaleString()}</p>
      ...
      {budgets.length > 0 && (overCount > 0 || warningCount > 0) && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-6 flex flex-wrap items-center gap-4">
          ...
          <div className="flex-1 text-sm">
            ...
            {overCount > 0 && (
              <span className="text-red-700 font-medium">{overCount} ngân sách vượt hạn mức</span>
            )}
            {overCount > 0 && warningCount > 0 && <span className="text-slate-500"> · </span>}
            {warningCount > 0 && (
              <span className="text-amber-700 font-medium">{warningCount} sắp chạm hạn mức (&gt;80%)</span>
            )}
          </div>
            ...
            <div className="flex flex-wrap gap-3 items-center mb-4">
              ...
              <div className="flex border border-slate-200 rounded-lg overflow-hidden ml-auto">
                <button
                  ...
                  className={`p-2 ${viewMode === "grid" ? "bg-purple-600 text-white" : "bg-white text-slate-500 hover:bg-slate-50"}`}
                  title="Grid view"
                >
                  <LayoutGrid size={16} />
                </button>
                <button
                  ...
                  className={`p-2 ${viewMode === "list" ? "bg-purple-600 text-white" : "bg-white text-slate-500 hover:bg-slate-50"}`}
                  title="List view"
                >
                  <List size={16} />
                </button>
              ...
              <div className="mt-3 space-y-2">
                {pieData.slice(0, 5).map((d) => (
                  <div key={d.name} className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-slate-700 shrink-0 ml-2">${d.value.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {editingBudget && (
        <EditBudgetModal budget={editingBudget} onClose={() => setEditingBudget(null)} onSave={handleEditBudget} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-200 hover:shadow-md transition-shadow group">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">${b.spent.toLocaleString()} / ${b.budget.toLocaleString()}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(b)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700" title="Sửa">
            <Pencil size={13} />
          </button>
          <button onClick={() => onDelete(b.id, b.name)} className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500" title="Xóa">
            <Trash2 size={13} />
          </button>
        </div>
      </div>
      ...
      <div className="w-full bg-slate-100 rounded-full h-2.5 mb-2">
        <div
          className={`h-2.5 rounded-full transition-all duration-700 bg-gradient-to-r ${
            isOver ? "from-red-500 to-red-600" : isWarning ? "from-yellow-400 to-orange-500" : b.barColor
          }`}
          style={{ width: `${pct}%` }}
        />
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400">
          {b.periodType && <span className="capitalize">{b.periodType === 'monthly' ? 'Hàng tháng' : b.periodType === 'weekly' ? 'Hàng tuần' : b.periodType === 'yearly' ? 'Hàng năm' : b.periodType}</span>}
          {b.startDate && (
    ...
    <div className="bg-white rounded-xl px-5 py-4 border border-slate-200 hover:shadow-sm transition-shadow group flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <StatusBadge percentage={b.percentage} />
          {b.periodType && <span className="text-xs text-slate-400 capitalize">{b.periodType === 'monthly' ? 'Hàng tháng' : b.periodType === 'weekly' ? 'Hàng tuần' : b.periodType === 'yearly' ? 'Hàng năm' : b.periodType}</span>}
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="text-right shrink-0">
        <p className="text-sm font-semibold text-slate-900">${b.spent.toLocaleString()}</p>
        <p className="text-xs text-slate-400">trên ${b.budget.toLocaleString()}</p>
      </div>
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => onEdit(b)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700">
          <Pencil size={13} />
        </button>
        <button onClick={() => onDelete(b.id, b.name)} className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500">
          <Trash2 size={13} />
        </button>
      </div>
    </div>
