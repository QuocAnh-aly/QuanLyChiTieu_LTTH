import { ArrowUpRight, ArrowDownRight, ArrowLeftRight, TrendingUp, RefreshCw, ExternalLink, X, MousePointerClick, Wallet, Landmark, PiggyBank } from "lucide-react";
import { BarChart, Bar, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, AreaChart, Area, ReferenceLine } from "recharts";
...
function mapRecentTransaction(t) {
  ...
  const accountTypeIds = [...new Set(details.map(d => d.typeId).filter(Boolean))];
...
export function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async (silent = false) => {
    try {
      ...
      const recentSource = Array.isArray(recentRaw) && recentRaw.length > 0
        ? recentRaw
  ...
  const netTrend = useMemo(
    () => monthlyTrend.map(p => ({ month: p.month, net: (p.income ?? 0) - (p.expenses ?? 0) })),
    [monthlyTrend]
  );

  // Assets / savings / debt side-by-side comparison for the net-worth breakdown chart.
  const financialComposition = useMemo(() => ([
    { name: "Tài sản",   typeId: 1, value: summary.totalAssets,      fill: "#10b981" },
    { name: "Tiết kiệm", typeId: 3, value: summary.totalSavings,     fill: "#8b5cf6" },
    { name: "Nợ",        typeId: 2, value: summary.totalLiabilities, fill: "#ef4444" },
  ]), [summary.totalAssets, summary.totalSavings, summary.totalLiabilities]);

  const hasCompositionData =
    summary.totalAssets > 0 || summary.totalSavings > 0 || summary.totalLiabilities > 0;

  const savingsSaved  = piggyBanks.reduce((s, g) => s + g.saved,  0);
  const savingsTarget = piggyBanks.reduce((s, g) => s + g.target, 0);
  const savingsPct    = savingsTarget > 0 ? Math.min((savingsSaved / savingsTarget) * 100, 100) : 0;
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (activeFilter.kind === "category")
      return recentTransactions.filter(t => t.categoryName === activeFilter.value);
    if (activeFilter.kind === "flow") {
      if (activeFilter.value === "income")   return recentTransactions.filter(t => t.isIncome);
      if (activeFilter.value === "expense")  return recentTransactions.filter(t => !t.isIncome && !t.isTransfer);
      if (activeFilter.value === "transfer") return recentTransactions.filter(t => t.isTransfer);
    }
    if (activeFilter.kind === "accountType")
      return recentTransactions.filter(t => t.accountTypeIds?.includes(activeFilter.value));
  ...
  const applyFilter = useCallback((next) => {
    setActiveFilter(prev =>
      prev && prev.kind === next.kind && prev.value === next.value ? null : next
    );
  }, []);

  const filterByCategory = useCallback((name) => {
    if (!name) return;
    applyFilter({ kind: "category", value: name, label: `Danh mục: ${name}` });
  }, [applyFilter]);

  const filterByFlow = useCallback((value, label) => {
    applyFilter({ kind: "flow", value, label });
  }, [applyFilter]);

  const filterByAccountType = useCallback((value, label) => {
    applyFilter({ kind: "accountType", value, label });
  }, [applyFilter]);

  // Convenience flags for highlighting the active card/chart element.
  const isFlowActive = (v) => activeFilter?.kind === "flow" && activeFilter.value === v;
  const isTypeActive = (v) => activeFilter?.kind === "accountType" && activeFilter.value === v;
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-8">
        ...
        >
          <div className="flex items-center justify-between mb-2 sm:mb-4">
            <span className="text-muted-foreground text-xs sm:text-sm">Tài sản</span>
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-emerald-500/15 flex items-center justify-center">
              <Wallet size={16} className="text-emerald-600" />
            </div>
          </div>
          <p className="text-xl sm:text-3xl font-bold text-card-foreground mb-1 sm:mb-2 truncate">{fmt(summary.totalAssets)}</p>
          <p className="text-muted-foreground text-xs sm:text-sm">Tổng giá trị tài sản</p>
        </button>
        ...
        >
          <div className="flex items-center justify-between mb-2 sm:mb-4">
            <span className="text-muted-foreground text-xs sm:text-sm">Nợ phải trả</span>
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-red-500/15 flex items-center justify-center">
              <Landmark size={16} className="text-red-600" />
            </div>
          </div>
          <p className="text-xl sm:text-3xl font-bold text-red-600 mb-1 sm:mb-2 truncate">{fmt(summary.totalLiabilities)}</p>
          <p className="text-muted-foreground text-xs sm:text-sm">Tổng dư nợ hiện tại</p>
        </button>
        ...
        >
          <div className="flex items-center justify-between mb-2 sm:mb-4">
            <span className="text-muted-foreground text-xs sm:text-sm">Lợn tiết kiệm</span>
          ...
          <p className="text-xl sm:text-3xl font-bold text-purple-600 mb-1 sm:mb-2 truncate">{fmt(summary.totalSavings)}</p>
          <p className="text-muted-foreground text-xs sm:text-sm">
            {piggyBanks.length > 0 ? `${piggyBanks.length} mục tiêu đang tiết kiệm` : "Đã để dành"}
          </p>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <h2 className="text-base sm:text-xl font-bold text-card-foreground">Thu nhập và Chi tiêu</h2>
          ...
          {monthlyTrend.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={monthlyTrend}>
                ...
                <Bar
                  dataKey="income" fill="#10b981" radius={[8, 8, 0, 0]} className="cursor-pointer"
                  fillOpacity={activeFilter?.kind === "flow" && activeFilter.value !== "income" ? 0.35 : 1}
                  onClick={() => filterByFlow("income", "Dòng tiền: Thu nhập")}
                />
                <Bar
                  dataKey="expenses" fill="#ef4444" radius={[8, 8, 0, 0]} className="cursor-pointer"
                  fillOpacity={activeFilter?.kind === "flow" && activeFilter.value !== "expense" ? 0.35 : 1}
                  onClick={() => filterByFlow("expense", "Dòng tiền: Chi tiêu")}
                />
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <h2 className="text-base sm:text-xl font-bold text-card-foreground">Chi tiêu theo danh mục</h2>
            <span className="hidden sm:flex items-center gap-1 text-xs text-muted-foreground">
      ...
      <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border mb-6 sm:mb-8">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <h2 className="text-base sm:text-xl font-bold text-card-foreground">Dòng tiền ròng theo tháng</h2>
          <span className="text-xs text-muted-foreground">Thu nhập − Chi tiêu</span>
        </div>
        {netTrend.some(p => p.net !== 0) ? (
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={netTrend}>
              ...
              />
              <Area type="monotone" dataKey="net" stroke="#8b5cf6" strokeWidth={2} fill="url(#netGradient)" />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
          <div className="h-[260px] flex items-center justify-center text-muted-foreground">Không có dữ liệu dòng tiền</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <h2 className="text-base sm:text-xl font-bold text-card-foreground">Cơ cấu tài chính</h2>
            <span className="hidden sm:flex items-center gap-1 text-xs text-muted-foreground">
              <MousePointerClick size={13} /> Nhấp để lọc
            </span>
          </div>
          {hasCompositionData ? (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={financialComposition} layout="vertical" margin={{ left: 8, right: 16 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="var(--color-border)" />
                <XAxis type="number" stroke="var(--color-muted-foreground)" tick={{ fontSize: 12 }} tickFormatter={fmtShort} />
                <YAxis type="category" dataKey="name" stroke="var(--color-muted-foreground)" tick={{ fontSize: 13 }} width={72} />
                <Tooltip
                  cursor={{ fill: "var(--color-muted)" }}
                  contentStyle={{ backgroundColor: "var(--color-card)", border: "1px solid var(--color-border)", borderRadius: "8px", color: "var(--color-card-foreground)" }}
                  formatter={(value) => [fmt(value), "Số tiền"]}
                />
                <Bar
                  dataKey="value" radius={[0, 8, 8, 0]} barSize={28} className="cursor-pointer"
                  onClick={(d) => d?.payload && filterByAccountType(d.payload.typeId, `Tài khoản: ${d.payload.name}`)}
                >
                  {financialComposition.map((entry) => (
                    <Cell
                      key={entry.name}
                      fill={entry.fill}
                      fillOpacity={activeFilter?.kind === "accountType" && activeFilter.value !== entry.typeId ? 0.35 : 1}
                    />
                  ))}
                </Bar>
              </BarChart>
            ...
            <div className="h-[240px] flex items-center justify-center text-muted-foreground">Chưa có dữ liệu tài sản</div>
          )}
        </div>
        ...
        <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border flex flex-col">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <h2 className="text-base sm:text-xl font-bold text-card-foreground">Tiến độ lợn tiết kiệm</h2>
            <Link
              to="/piggy-banks"
          ...
          {piggyBanks.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
              ...
              <p className="text-muted-foreground text-sm">Chưa có lợn tiết kiệm nào.</p>
              <Link to="/piggy-banks" className="mt-2 text-sm text-purple-600 hover:text-purple-700 font-medium">
            ...
            <>
              {savingsTarget > 0 && (
                <div className="mb-4 pb-4 border-b border-border">
                  <div className="flex items-center justify-between text-sm mb-1.5">
                    ...
                    <span className="font-semibold text-foreground">{fmt(savingsSaved)} / {fmt(savingsTarget)}</span>
                  </div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div className="h-full rounded-full bg-purple-500" style={{ width: `${savingsPct}%` }} />
                  </div>
                </div>
              )}
              <div className="space-y-4">
                {piggyBanks.slice(0, 4).map((g) => (
                  <div key={g.id}>
                    <div className="flex items-center justify-between text-sm mb-1.5">
                      <span className="font-medium text-card-foreground truncate max-w-[55%]">{g.name}</span>
      ...
      <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border">
        ...
        {recentTransactions.length === 0 ? (
          ...
          <div className="text-center py-8">
            <p className="text-muted-foreground">Không có giao dịch nào khớp với bộ lọc.</p>
            <button
              type="button"
              onClick={() => setActiveFilter(null)}
