...
export const colorMap = {
  ...
  slate:   { text: "text-muted-foreground",   bg: "bg-muted",   bar: "from-slate-400 to-slate-600",    pie: "#64748b" },
};
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-card rounded-2xl p-6 border border-border">
          <div className="flex items-center justify-between mb-4">
            <span className="text-muted-foreground text-sm font-medium">Tổng ngân sách</span>
            <TrendingUp size={20} className="text-muted-foreground" />
          </div>
          <p className="text-3xl font-bold text-card-foreground">{fmt(totalBudget)}</p>
          <p className="text-muted-foreground text-sm mt-1">{budgets.length} ngân sách đang hoạt động</p>
        </div>

        <div className="bg-card rounded-2xl p-6 border border-border">
          <div className="flex items-center justify-between mb-4">
            <span className="text-muted-foreground text-sm font-medium">Tổng chi tiêu</span>
            <div className="w-2 h-2 rounded-full bg-red-500" />
          </div>
          <p className="text-3xl font-bold text-card-foreground">{fmt(totalSpent)}</p>
      ...
      {budgets.length > 0 && (overCount > 0 || warningCount > 0) && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-6 flex flex-wrap items-center gap-4">
          ...
          <div className="flex-1 text-sm">
            ...
            {overCount > 0 && warningCount > 0 && <span className="text-muted-foreground"> · </span>}
      ...
      {budgets.length > 0 && (
        <div className="bg-card rounded-2xl p-6 border border-border mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-sm font-semibold text-muted-foreground">{overallPct.toFixed(1)}%</span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-3 items-center mb-4">
              ...
              <div className="flex border border-border rounded-lg overflow-hidden ml-auto">
                <button onClick={() => setViewMode("grid")} className={`p-2 ${viewMode === "grid" ? "bg-purple-600 text-white" : "bg-card text-muted-foreground hover:bg-muted"}`}><LayoutGrid size={16} /></button>
                <button onClick={() => setViewMode("list")} className={`p-2 ${viewMode === "list" ? "bg-purple-600 text-white" : "bg-card text-muted-foreground hover:bg-muted"}`}><List size={16} /></button>
              </div>
            </div>
            ...
            <div className="w-full lg:w-64 shrink-0 bg-card rounded-2xl p-5 border border-border self-start">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: "var(--color-card)", border: "1px solid var(--color-border)", borderRadius: "10px", fontSize: "12px" }}
      ...
      {!isLoading && budgets.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 bg-card rounded-2xl border border-dashed border-border">
          ...
          <p className="text-muted-foreground font-semibold text-lg">Chưa có ngân sách</p>
          <p className="text-muted-foreground text-sm mt-1 mb-6">Hãy tạo ngân sách đầu tiên để bắt đầu theo dõi chi tiêu</p>
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-card-foreground leading-tight hover:text-purple-600 transition-colors">{b.name}</h3>
            <p className="text-xs text-muted-foreground mt-0.5">{fmt(b.spent)} / {fmt(b.budget)}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={(e) => { e.stopPropagation(); onEdit(b); }} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground" title="Sửa"><Pencil size={13} /></button>
          <button onClick={(e) => { e.stopPropagation(); onDelete(b.id, b.name); }} className="p-1.5 rounded-lg hover:bg-red-50 text-muted-foreground hover:text-red-500" title="Xóa"><Trash2 size={13} /></button>
        </div>
      </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="font-semibold text-card-foreground text-sm hover:text-purple-600 transition-colors">{b.name}</span>
          <StatusBadge percentage={b.percentage} />
          {b.periodType && <span className="text-xs text-muted-foreground">{b.periodType === "monthly" ? "Hàng tháng" : b.periodType === "weekly" ? "Hàng tuần" : b.periodType === "yearly" ? "Hàng năm" : b.periodType}</span>}
        </div>
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground shrink-0 w-10 text-right">{b.percentage.toFixed(0)}%</span>
        </div>
      </div>
      <div className="text-right shrink-0">
        <p className="text-sm font-semibold text-card-foreground">{fmt(b.spent)}</p>
        <p className="text-xs text-muted-foreground">trên {fmt(b.budget)}</p>
      </div>
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={(e) => { e.stopPropagation(); onEdit(b); }} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground"><Pencil size={13} /></button>
        <button onClick={(e) => { e.stopPropagation(); onDelete(b.id, b.name); }} className="p-1.5 rounded-lg hover:bg-red-50 text-muted-foreground hover:text-red-500"><Trash2 size={13} /></button>
      </div>
    </div>
  );
}
