...
const PIE_COLORS = ["#22c55e", "#10b981", "#3b82f6", "#8b5cf6", "#06b6d4", "#f97316", "#eab308", "#ec4899", "#14b8a6", "#6366f1"];
...
export function IncomeView() {
  ...
  const incomes = useMemo(() => allTx.filter(t => {
    const details = t.details || [];
  ...
  const sourceData = useMemo(() => {
    ...
    incomes.forEach(t => map.set(t.categoryName, { value: (map.get(t.categoryName)?.value || 0) + t.totalAmount, hasIncome: true }));
    ...
    return [...map.entries()]
      .map(([name, data]) => ({ name, value: data.value, hasIncome: data.hasIncome }))
  ...
  if (isLoading) {
    return (
      <PageLayout
        ...
        subtitle={<span className="ml-[52px]">Xem tổng quan các khoản thu — theo dõi nguồn thu nhập của bạn</span>}
      >
  ...
  return (
    <PageLayout
      ...
      subtitle={
        <span className="ml-[52px]">Xem tổng quan các khoản thu — theo dõi nguồn thu nhập của bạn</span>
      }
      actions={
        <button
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6">
        {PRESETS.map(p => (
          <button key={p.key} onClick={() => { setPreset(p.key); setShowCustom(false); }}
            className={`px-3.5 py-1.5 rounded-full text-sm font-medium border transition-colors ${
              preset === p.key ? "bg-green-500 text-white border-green-500 shadow-sm" : "border-border text-muted-foreground hover:border-green-300 hover:text-green-600"
            }`}>
            {p.label}
          </button>
        ))}
        <button onClick={() => { setPreset("custom"); setShowCustom(true); }}
          className={`px-3.5 py-1.5 rounded-full text-sm font-medium border transition-colors ${
            preset === "custom" ? "bg-green-500 text-white border-green-500 shadow-sm" : "border-border text-muted-foreground hover:border-green-300 hover:text-green-600"
          }`}>
      ...
      {!isLoading && incomes.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-card rounded-2xl p-6 border border-border shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-xs text-muted-foreground mb-4">Phân bổ thu nhập theo từng nguồn</p>
          ...
          <div className="bg-card rounded-2xl p-6 border border-border shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-xs text-muted-foreground mb-4">Biến động thu nhập hằng ngày</p>
      ...
      <div className="bg-card rounded-2xl p-6 border border-border shadow-sm mb-6">
        ...
        <p className="text-xs text-muted-foreground mb-4">Các nguồn thu nhập đã thiết lập — nguồn có <span className="text-green-600 font-semibold">khoản thu</span> trong kỳ và nguồn <span className="text-muted-foreground">chưa có</span></p>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className={`text-sm font-semibold truncate ${isActive ? 'text-card-foreground' : 'text-muted-foreground'}`}>
                    {item.name}
                  </p>
                ...
                {selectedCategories.includes(item.name) && (
                  <span className="text-lg text-green-600 ml-auto shrink-0">&times;</span>
                )}
              </div>
            );
          })}
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            <TrendingUp size={48} className="mx-auto mb-3 text-muted-foreground opacity-30" />
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {selectedCategories.length > 0
                ? <button onClick={() => setSelectedCategories([])} className="text-green-500 hover:text-green-600 underline underline-offset-2">Xóa bộ lọc</button>
                : selectedDate
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-xs uppercase tracking-wider text-muted-foreground border-b border-border bg-muted/50">
                  ...
                  <th className="px-4 sm:px-6 py-3.5 font-semibold whitespace-nowrap">Nguồn thu</th>
                  <th className="px-4 sm:px-6 py-3.5 font-semibold whitespace-nowrap">Nhận vào ví</th>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map(t => (
                  <tr key={t.journalId} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-4 sm:px-6 py-3.5 text-sm text-muted-foreground">{t.destAccount}</td>
                    ...
                    <td className="px-4 sm:px-6 py-3.5 text-right font-bold text-sm whitespace-nowrap">
                      <span className="text-green-600">+{fmt(t.totalAmount)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
