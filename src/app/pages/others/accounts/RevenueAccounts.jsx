...
export function RevenueAccounts() {
  ...
  const fetchAccounts = useCallback(async () => {
    ...
    try {
      ...
      setAccounts([]);
      toast.error("Không thể tải dữ liệu tài khoản thu");
    } finally {
      setIsLoading(false);
    }
  }, []);
    ...
    >
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2].map(i => <div key={i} className="h-40 bg-accent animate-pulse rounded-2xl" />)}
        </div>
      ) : filtered.length === 0 ? (
        <div className="py-16 text-center bg-card rounded-2xl border border-border">
          <Landmark size={40} className="mx-auto text-muted-foreground mb-3" />
          <p className="text-muted-foreground font-medium">
            {search ? 'Không tìm thấy nguồn thu phù hợp' : 'Chưa có tài khoản thu nào'}
          </p>
          {!search && <p className="text-muted-foreground text-sm mt-1">Nhấn "Thêm nguồn thu" để bắt đầu</p>}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(acc => (
