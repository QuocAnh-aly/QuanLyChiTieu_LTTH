    ...
    >
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2].map(i => <div key={i} className="h-48 bg-accent animate-pulse rounded-2xl" />)}
        </div>
      ) : filtered.length === 0 ? (
        <div className="py-16 text-center bg-card rounded-2xl border border-border">
          <CheckCircle2 size={48} className="mx-auto text-green-500 mb-3" />
          <p className="text-card-foreground font-bold text-lg mb-1">Tài chính lành mạnh</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map(acc => {
            ...
            return (
              ...
              >
                <div className="absolute top-0 right-0 w-32 h-32 bg-card opacity-5 rounded-full -mr-10 -mt-10" />
