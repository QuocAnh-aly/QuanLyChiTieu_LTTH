...
const COLOR_OPTIONS = [
  ...
  { value: 'slate',   label: 'Xám',        bg: 'bg-slate-100',   text: 'text-slate-600',   swatch: 'bg-slate-500'   },
];
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        {filtered.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <p className="text-slate-900 font-bold text-lg mb-1">Chưa có thẻ nào</p>
            <p className="text-slate-500">Tạo thẻ để dễ dàng theo dõi các chiến dịch hoặc chuyến đi cụ thể.</p>
          </div>
        ) : (
          <div className="flex flex-wrap gap-3">
            {filtered.map(tag => {
              ...
              return (
                <div
                  key={tag.id}
                  className="group relative flex items-center gap-2 pr-2 pl-3 py-1.5 rounded-full border border-slate-200 bg-slate-50 hover:border-purple-300 hover:shadow-md transition-all"
                >
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              {form.name.trim() && (
                <div>
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 rounded-full border border-slate-200 bg-slate-50 w-fit">
                    <Hash size={14} className={(COLOR_MAP[form.color] || COLOR_MAP.slate).text} />
                    <span className="font-semibold text-slate-800 text-sm">{form.name.trim()}</span>
